(function ($) {
    ...
    MyJqDialog.prototype.init = function (element, options) {
        ...
        var overlay_css = {"width": "100%", "height": "100%", "filter": "alpha(opacity=40)",
            "-moz-opacity": "0.4", "-khtml-opacity": "0.4", "opacity": "0.4", "background": "#fff",
    ...
    MyJqMyConfirm.prototype = {
        init: function (options){
            ...
            width = width * 0.8;
            var defaults = {width: width+"px", cancelButton: true, confirmButton: true, title: "鎻愮ず",
                cancel_btn_title: "鍙栨秷", confirm_btn_title: "纭",
    ...
    MyJqMyLoad.prototype = {
        init: function (options){
            ...
            var width = $(document).width();
            width = width * 0.5;
            var defaults = {width: width+"px", title: "数据加载中..."};
            if(typeof options === 'undefined') options = {};
            this.settings = $.extend( {}, defaults, options );
